...
# def collect_fe_related(param_set, df):
#     param_set.reset()
...
#     return design_matrix, design_matrix_re, lbs_fe, ubs_fe, constr_matrix, constr_lower_bounds, constr_upper_bounds, fe_priors
...
def process_for_marginal(param_set, df):
    ...
    param_set.re_var_diag_matrix = block_diag(*re_var_diag)    

    # ---- checking dimensions ----
    assert param_set.design_matrix.shape[1] == param_set.num_fe
    ...
    assert param_set.design_matrix_re.shape[1] == param_set.re_var_diag_matrix.shape[0] == param_set.num_re
...
def process_for_maximal(param_set, df):
    ...
    assert constr_matrix.shape[1] == param_set.num_fe
    assert len(constr_lower_bounds) == len(constr_upper_bounds) == constr_matrix.shape[0] 
    assert param_set.design_matrix_re.shape[1] == param_set.num_re
...
#     constr_mat_re_blocks = collect_blocks(param_set, 'constr_matrix_re', 'build_constraint_matrix_re', should_include=lambda v: v.add_re == True)
#     constr_lbs = collect_blocks(param_set, 'constr_lb')
#     constr_ubs = collect_blocks(param_set, 'constr_ub')
#     constr_lbs_re = collect_blocks(param_set, 'constr_lb_re', should_include=lambda v: v.add_re == True)
#     constr_ubs_re = collect_blocks(param_set, 'constr_ub_re', should_include=lambda v: v.add_re == True)
#     fe_priors = collect_blocks(param_set, 'fe_prior')
#     re_priors = collect_blocks(param_set, 're_prior', should_include=lambda v: v.add_re == True)
#     grouping = collect_blocks(param_set, 'n_groups', should_include=lambda v: v.add_re == True)
...
#     param_set.design_matrix_re = np.hstack(design_mat_re_blocks)
#     constr_matrix_re = block_diag(*constr_mat_re_blocks)
...
#     param_set.constr_matrix_full = block_diag(constr_matrix, constr_matrix_re)
#     param_set.constr_lower_bounds_full = np.hstack((constr_lower_bounds, constr_lower_bounds_re))
#     param_set.constr_upper_bounds_full = np.hstack((constr_upper_bounds, constr_upper_bounds_re))

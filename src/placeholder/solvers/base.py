...
class ScipyOpt(Solver):
    ...
    def fit(self, data, x_init=None, options=None):
        if x_init is None:
            if self._model.x_init is not None:
                x_init = self._model.x_init 
            else:
                raise ValueError('No initial value for x.')
        result = sciopt.minimize(
            fun=lambda x: self._model.objective(x, data),
            x0=x_init,
            jac=lambda x: self._model.gradient(x, data),
    ...
    def predict(self, **kwargs):
        return self._model.predict(self.x_opt, **kwargs)
